<template>
  <div class="home">
    <PhotoGame :props="{vertical,gorisontal,sideAmount,boxWidth,boxHeight}" 
...
export default {
  ...
  methods: {
    ...
    recaunt() {
      setTimeout(()=> {
        for(let x = 0; x < cont.children.length; x++) {
          ...
          let dataY = this.gorisontalPositions[+cont.children[x].getAttribute('data-pos')]
    ...
    animate() {
      for(let x = 0; x < cont.children.length; x++) {
        cont.children[x].style.animation = `animation${this.aminationIterate} 2s ease ${x/100}s`
        console.log(x)
      }
